    ...
    async function fetchResponse(message) {
        try {
            const response = await fetch("https://api.openai.com/v1/chat/completions", {
                method: "POST",
                headers: {
                    organization: "org-vDhLW8rLfpwrC2vyomFEipR6",
                    "Content-Type": "application/json",
                    "Authorization": `Bearer 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`
                },
                body: JSON.stringify({
                    model: "gpt-3.5-turbo",
                    messages: [
            ...
            if (!response.ok) {
                ...
                console.error("Error: Failed to fetch response from OpenAI API", response.status, response.statusText, errorData);
            ...
            if (data.choices && data.choices.length > 0) {
                ...
                console.error("API returned an empty response or unexpected data format:", data);
